...
public partial class VirtualWorld
{
    ...
    private T Edit<T>(IVReadOnlyInfo rinfo, string md) where T : VInfo
    {
        if (rinfo is not T info)
            throw new Exception($"VIRTUAL NODE IS NOT {typeof(T).Name.ToUpper()[1..]}: {md}");
        if (IsReadOnly(rinfo.Id))
            throw new Exception($"CANNOT EDIT READ-ONLY {typeof(T).Name.ToUpper()[1..]}: {md}");

        return info;
    }
    private bool TryEdit<T>(IVReadOnlyInfo rinfo, out T info) where T : VInfo
    {
        info = null!;
        if (IsReadOnly(rinfo.Id) || rinfo is not T einfo)
            return false;
    ...
    internal void SetReadOnly() => _readOnlyCount = _nodeCount;
    public bool IsReadOnly(int nodeId) => _readOnlyCount > nodeId;
   ...
   => _nodeInfos[nodeId];
    ...
    public bool IsEditable(int nodeId) => !IsReadOnly(nodeId);
    public VInfo EditInfoAt(int nodeId)
    {
        if (IsReadOnly(nodeId))
            throw new Exception($"CANNOT EDIT READ-ONLY INFO, USE \"ReadInfoAt()\" INSTEAD");

        return _nodeInfos[nodeId];
    }
    public T EditInfoAt<T>(int nodeId) where T : VInfo
    {
        if (IsReadOnly(nodeId))
            throw new Exception($"CANNOT EDIT READ-ONLY {typeof(T).Name[1..].ToUpperInvariant()}, USE \"ReadInfoAt<{typeof(T).Name}>()\" INSTEAD");

        return (T)_nodeInfos[nodeId];
    }
    public bool TryEditInfoAt<T>(int nodeId, out T info) where T : VInfo
    {
        info = null!;
        if (IsReadOnly(nodeId) || _nodeInfos[nodeId] is not T eInfo)
            return false;
    ...
    //===== BASE ASSEMBLIES =====
    public const int CORELIB_HASH = 0;
    public IVReadOnlyAssembly Corelib => ReadAssembly(CORELIB_HASH);

    public int PROJECT_HASH => _assemblies.Count - 1;
    public IVReadOnlyAssembly Project => ReadAssembly(PROJECT_HASH);

    //**ASSEMBLY**
    readonly List<int> _assemblies = [];
    public IReadOnlyCollection<int> Assemblies => _assemblies;

    public IVReadOnlyAssembly ReadAssembly(int asmHash)
    {
        if (0 > asmHash || asmHash >= _assemblies.Count)
            throw new Exception($"ASSEMBLY HASH NOT FOUND: asmHash={asmHash}");

        return ReadInfoAt<VAssembly>(_assemblies[asmHash]);
    }
    public VAssembly EditAssembly(int asmHash)
    => Edit<VAssembly>(ReadAssembly(asmHash), $"asmHash={asmHash}");
    public bool TryEditAssembly(int asmHash, out VAssembly info)
    => TryEdit(ReadAssembly(asmHash), out info);

    internal VAssembly AddAssembly()
    {
        var hash = _assemblies.Count;

        var info = new VAssembly(hash);
        var asmId = _assemblies[hash] = AddNode(VKind.Assembly, info);

        var gspaceInfo = new VNspace("");
        info.GlobalNspace = AddNode(VKind.Nspace, gspaceInfo, asmId);
...
//>>>> VASSEMBLY <<<<
public interface IVReadOnlyAssembly : IVReadOnlyInfo
{
    public int Hash { get; }

    //SCHEME
    public int GlobalNspace { get; }
}
public sealed class VAssembly : VInfo, IVReadOnlyAssembly
{
    public int Hash { get; }
    ...
    //SCHEME
    public int GlobalNspace { get; internal set; }
}
...
//===========================
//   VIRTUAL TYPE MEMBERS
//===========================
public interface IVReadOnlyTypeMember : IVReadOnlyMember { }
public abstract class VTypeMember : VMember, IVReadOnlyTypeMember
{ internal VTypeMember(string name) : base(name) { } }