...
{
    public static void Debug(DzProject project)
    {
        var vWorld = project.VWorld;
        foreach (var (asmName, asmHash) in project._assemblyByName)
        {
            Console.WriteLine($"======= {asmName} =======");
            DebugAssembly(vWorld, asmHash);
        }
    }
    ...
    private static void DebugAssembly(VirtualWorld vWorld, int asmHash)
    {
        var rootId = vWorld.ReadAssembly(asmHash).GlobalNspace;

        //PRINT GRAPH
        void debug(in VNode node, int tabs)
        {
            var info = vWorld.ReadInfoAt(node.Id);
            ...
            while (childId >= 0)
            {
                ref readonly var child = ref vWorld.NodeAt(childId);
                debug(child, tabs + 1);

                childId = child.NextSiblingId;
            }
        }
        debug(vWorld.NodeAt(rootId), 0);
    }
}