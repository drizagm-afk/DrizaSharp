...
public class Render
{
    ...
    private void Debug()
    {
        ...
        //LEXER
        {
            ...
            if (showStruct)
                DebugTokens();
            if (showLogs)
                DebugLogs("Lexer", File.LexerDiagnostics);
        }
        //PARSER
        {
            ...
            if (showStruct)
                DebugTAST();
            if (showLogs)
                DebugLogs("Parser", File.ParserDiagnostics);
        }
        //LOWERER
        {
            ...
            if (showStruct)
                DebugTASI();
            if (showLogs)
                DebugLogs("Lowerer", File.LowererDiagnostics);
        }
    }
    ...
    private void DebugTokens()
    {
        ...
        for (int i = 0; i < TAST.TokenCount; i++)
        {
            ...
            string cont = $"[{token.Id:D3} {Rules.Lexer.Ruleset.TokenName(Project, token.Type)}] ";
            if (token.Type != Tokens.NULL && token.Type != Tokens.NEWLINE)
    ...
    //DEBUG LOGS
    private void DebugLogs<T>(string group, GroupDiagnostics<T> diag) where T : struct
    {
        Write(">> COMPILER LOGS: ");
        if (diag.Reports.Count <= 0)
            WriteLine("NONE\n");

        List<DiagnosticEntry<T>> sysEntries = [];
        List<DiagnosticEntry<T>> entries = [];
    ...
    private void DebugTitle<T>(string group, DiagnosticEntry<T> entry) where T : struct
    {
...
public class DebugSource
{
    ...
    public DebugSource(SourceText source, StringBuilder stream)
    {
        Stream = stream;
