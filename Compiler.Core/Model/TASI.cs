...
public sealed class TASI
{
    ...
    public int NewInstruction(byte typeId, int start, int length)
    {
        var off = _instCount++;
        if (_instructions.Length <= _instCount)
            Array.Resize(ref _instructions, _instructions.Length * 2);

        _instructions[off] = new(typeId, start, length);
        return off;
    }
    ...
    private int NewNode(TASIArgs args, int relIndex, int start, int length)
    {
        var id = _nodeCount++;
        if (_nodes.Length <= _nodeCount)
            Array.Resize(ref _nodes, _nodes.Length * 2);

        _nodes[id] = new(id, args, relIndex, start, length, -1, -1);
        return id;
    }
    ...
    public int AddNode(int start, int length, SchemeTASIArgs args)
    => AddNode(0, 0, start, length, args);
    public int AddNode(int parentId, int index, int start, int length, SchemeTASIArgs args)
    {
        ref readonly var parent = ref NodeAt(parentId);
        int nestId = NewNode(parent.Args.With(args), index, start, length);
...
//===== INSTRUCTIONS =====
public readonly struct Instruction(byte typeId, int start, int length)
{
